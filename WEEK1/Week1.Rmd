...
c <- a-b
c <- a*b
c <- a/b # no need to worry about int vs. float!
class(c)
...
```
```{r.}
## create a vector (R likes to call them lists)
x <- c(1,1,2,3,5,8) # all numbers


## create a range of values
r <- seq(1,10)
r <- 1:10 # : works too, but no step option
r <- seq(1,10,by=2) # seq includes a step size with 'by' argument

## index into a vector
x[1]
x[3]
x[1:3] # : range of indices
x[c(1,3,4)] # index with vector
x[c(1:3,6)] # can include ranges and single indices
x < 3 # standard logical operations 
x[x<3] # ...can be used for indexing
